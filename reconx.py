...
def scan_port(ip, port, timeout, results, lock, grab):
...
def worker(ip, q, timeout, results, lock, grab, progress):
    while not q.empty():
        try:
            port = q.get_nowait()
        except Exception:
...
def run_scan(target, ports, threads, timeout, grab):
    ...
    for _ in range(min(threads, total or 1)):
        t = threading.Thread(target=worker, args=(ip, q, timeout, results, lock, grab, progress))
        t.daemon = True
        t.start()
...
def print_results(ip, results):
    ...
    print(f"{C.BOLD}{'PORT':<8}{'STATE':<8}{'SERVICE':<15}{'BANNER'}{C.END}")
    print("-" * 60)
    for r in results:
        banner = r["banner"] if r["banner"] else "-"
        print(f"{r['port']:<8}{C.GREEN}{'open':<8}{C.END}{r['service']:<15}{banner}")
...
EPILOG = """
examples:
  reconx 192.168.1.1                       scan top 1000 ports (default)
  reconx example.com -p 22,80,443          scan specific ports
  reconx 10.0.0.5 -p 1-65535 -t 200        full port range, 200 threads
  reconx 192.168.1.1 -b                    grab service banners
  reconx 192.168.1.1 -X                    full scan: banners + OS guess (like nmap -A)
  reconx 192.168.1.1 -p 1-1000 -o out.json save results to a file
...
def main():
    ...
    parser.add_argument("-p", "--ports", default="1-1000",
                         help="Ports to scan, e.g. '80,443' or '1-1000' (default: 1-1000)")
    ...
    try:
        ip, results, elapsed = run_scan(args.target, ports, args.threads, args.timeout, args.banner)
